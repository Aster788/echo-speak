...
Sync curated English notes from Feishu into Echo Speak (Phase 6): per-user credentials, structure-aware doc parse, dual ingest (LLM sentences + rule tables), silent Home auto-sync, and manual Sync entry points. Feishu remains source of truth — Echo reads and mirrors, never overwrites notes.
...
The system SHALL list accessible Feishu cloud documents and fetch text or Markdown content for each document selected for sync. Explicit docx tokens from `user_settings.feishu_document_urls` SHALL be merged with drive listing results.

#### Scenario: Fetch note content

- **WHEN** sync processes a document token returned by the list API or configured URLs
...
The system SHALL parse vocabulary tables without an LLM, pairing adjacent English and Chinese cells into expressions. Table lemmas SHALL NOT invent `example_en` by copying the phrase; `example_en` / `example_zh` SHALL be null when no real example sentence exists.
...
#### Scenario: Table lemma has no fake example

- **WHEN** a table vocab item is persisted
- **THEN** `example_en` and `example_zh` are null (phrase and meaning only)
...
The system SHALL extract structured expressions from `bullet` and `phrase` blocks using an LLM prompt for sentence-level notes. The current Section label and `videoTitle` MAY be passed as extract context. The LLM SHALL NOT assign Echo Topics. When the model returns an `example_en` that is not distinct from the lemma/phrase, the system SHALL store `example_en` as null.
...
- **WHEN** an extracted expression has a distinct `example_en` but no Chinese example
...
The system SHALL expose manual Feishu sync via Settings (optional), `POST /api/feishu/sync`, and `scripts/sync-feishu.ts` CLI using the same core service. Long Sync all runs SHALL be chunked client-side so each server invocation stays under platform timeouts.
