...
The system SHALL persist transcript content in a `transcripts` table linked to a video via `video_id` (uuid FK → videos.id), with `raw_text` (text), `cleaned_text` (text, nullable), and `created_at` (timestamptz).
