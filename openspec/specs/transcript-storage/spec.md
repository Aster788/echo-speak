...
Persist video metadata and transcript content in Supabase for Echo Speak Phase 1 (Foundation), including Feishu-linked video rows (Phase 6).

## Requirements

### Requirement: Video metadata storage

The system SHALL persist video metadata in a `videos` table with columns: `id` (uuid), `title` (text), `youtube_url` (text, optional), `creator` (text, nullable), `source` (text: `youtube`, `manual`, or `feishu`), and `created_at` (timestamptz).
